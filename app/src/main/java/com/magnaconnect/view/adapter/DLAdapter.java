...
public class DLAdapter extends RecyclerView.Adapter<DLAdapter.DataObjectHolder> implements Filterable {
    private static String TAG = DLAdapter.class.getSimpleName();
    private static MyClickListener myClickListener;
    public List<VerifyResponse> gridList;
    public Activity activity;
    private List<VerifyResponse> gridListFiltered;
    private ItemFilter mFilter = new ItemFilter();

    public DLAdapter(Activity activity, List<VerifyResponse> gridList) {
    ...
    public void onBindViewHolder(final DataObjectHolder holder, final int position) {
        try {
            VerifyResponse item = gridList.get(position);

            holder.S_No_TV.setText((position + 1) + ".");

            holder.titleTV.setText(item.getUserName());
...
//            mSquareView = itemView.findViewById(R.id.mSquareView);
            ...
            Utility.disableVisible( buttonContainer);
        ...
        protected FilterResults performFiltering(CharSequence charSequence) {
            ...
            if (charString.isEmpty()) {
                ...
                for (VerifyResponse row : gridList) {
                    ...
                    if (row.getUserName()!= null && row.getUserName().toLowerCase().contains(charString.toLowerCase()) || row.getMobileNumber().contains(charSequence)) {
